...
class SeixalImage(Dataset):
    ...
    def __init__(self):
        ...
        self.height = 1796
        self.width = 4973
        self.patches = np.load('p_all.npy')
        self.mean = np.mean(self.patches)
        self.std = np.std(self.patches)

    def __len__(self):
        return (self.height // self.crop_x) * (self.width // self.crop_y)

    def __getitem__(self, patch_index):
        patches = torch.from_numpy(self.patches[patch_index, :, :])
    ...
    def __init__(self):
        ...
        self.encode = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=5,
                      padding=2), nn.BatchNorm2d(16), nn.MaxPool2d(2),
            nn.Conv2d(in_channels=16,
                      out_channels=32,
                      kernel_size=5,
                      padding=2), nn.BatchNorm2d(32), nn.MaxPool2d(2),
            nn.Conv2d(in_channels=32,
                      out_channels=64,
                      kernel_size=5,
                      padding=2), nn.BatchNorm2d(64), nn.MaxPool2d(2),
            nn.Conv2d(in_channels=64,
                      out_channels=128,
                      kernel_size=5,
                      padding=2), nn.BatchNorm2d(128), nn.MaxPool2d(2),
            nn.Conv2d(in_channels=128,
                      out_channels=256,
                      kernel_size=5,
                      padding=2), nn.BatchNorm2d(256), nn.MaxPool2d(2))
    ...
    def __init__(self):
        ...
        self.decode = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=256,
                               out_channels=128,
                               kernel_size=5,
                               padding=2), nn.BatchNorm2d(128),
            nn.Upsample(scale_factor=2),
            nn.ConvTranspose2d(in_channels=128,
                               out_channels=64,
                               kernel_size=5,
                               padding=2), nn.BatchNorm2d(64),
            nn.Upsample(scale_factor=2),
            nn.ConvTranspose2d(in_channels=64,
                               out_channels=32,
                               kernel_size=5,
                               padding=2), nn.BatchNorm2d(32),
            nn.Upsample(scale_factor=2),
            nn.ConvTranspose2d(in_channels=32,
                               out_channels=16,
                               kernel_size=5,
                               padding=2), nn.BatchNorm2d(16),
            nn.Upsample(scale_factor=2),
            nn.ConvTranspose2d(in_channels=16,
                               out_channels=1,
                               kernel_size=5,
                               padding=2), nn.BatchNorm2d(1))
...
def train_model(_encoder, _decoder, _train_dataloader):
    criterion = nn.MSELoss(reduction='mean')
    encoder_optimizer = torch.optim.SGD(_encoder.parameters(),
                                        lr=0.001,
                                        momentum=0.9)
    decoder_optimizer = torch.optim.SGD(_decoder.parameters(),
                                        lr=0.001,
                                        momentum=0.9)
    ...
    for _ in tqdm(range(epochs)):
        ...
        epoch_loss.append(curr_train_loss)
